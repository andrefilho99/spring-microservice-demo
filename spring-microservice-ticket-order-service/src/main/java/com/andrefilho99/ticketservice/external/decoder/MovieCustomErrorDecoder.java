...
@Log4j2
public class MovieCustomErrorDecoder implements ErrorDecoder {
    @Override
    public Exception decode(String methodKey, Response response) {
        ...
        if(response.status() == HttpStatus.SERVICE_UNAVAILABLE.value()) {
            throw new ServiceNotAvailableException("Movie service is down!");
        } else {
            ...
            switch (statusCode) {
                case 400:
                    return new SpotsException(errorMessage != null ? errorMessage : "Bad request.");
                case 404:
